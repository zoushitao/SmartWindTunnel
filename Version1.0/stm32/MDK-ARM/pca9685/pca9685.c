...
bool pca9685_init(pca9685_handle_t *handle)
{
	...
	uint8_t mode1_reg_dvalue ;
	 mode1_reg_dvalue = 0x30;
	uint8_t mode2_reg_dvalue = 0x00;

	if (handle->inverted) {
		mode2_reg_dvalue |= 0x10;
	}

	success &= pca9685_write_u8(handle, PCA9685_REGISTER_MODE1, mode1_reg_dvalue);
	success &= pca9685_write_u8(handle, PCA9685_REGISTER_MODE2, mode2_reg_dvalue);
